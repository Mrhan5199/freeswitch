...
function public.encryptString(key, data, modeFunction)
    ...
    for i = 1, #data/16 do
		...
		local byteDatach = "";
		for k,v in pairs(byteData) do byteDatach = byteDatach..string.char(v); end
        buffer.addString(encryptedData, byteDatach); 
...
function public.decryptString(key, data, modeFunction)
    ...
    for i = 1, #data/16 do
		...
		local byteDatach = "";
		for k,v in pairs(byteData) do byteDatach = byteDatach..string.char(v); end
        buffer.addString(decryptedData, byteDatach); 
